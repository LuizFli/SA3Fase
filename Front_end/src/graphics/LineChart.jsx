...
const LineChart = () => {
  ...
  const [categories, setCategories] = useState(['Jan', 'Fev', 'Mar', 'Abr', 'Mai', 'Jun']); 
  ...
  const handlePeriodChange = (event) => {
    ...
    if (newPeriod === 'Mês') {
      ...
      newData = {
        ganhos: Array.from({ length: 6 }, () => Math.floor(Math.random() * 1000)), 
        previsto: Array.from({ length: 6 }, () => Math.floor(Math.random() * 1000)),
      };
      newCategories = Array.from({ length: 6 }, (_, i) => (currentYear - 5 + i).toString()); 
    }
  ...
  const receitaFormatada =
    selectedPeriod === 'Mês'
      ? `R$ ${totalReceita}K` 
      : `R$ ${(totalReceita / 1000).toFixed(1)}M`; 
